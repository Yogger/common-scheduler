...
import com.windforce.common.scheduler.Scheduler;
import com.windforce.common.scheduler.Trigger;
import com.windforce.common.utility.thread.NamedThreadFactory;
...
public class SimpleScheduler implements Scheduler {
	...
	private static class LogDecorateTask implements ScheduledTask {
		...
		public void run() {
			if (logger.isInfoEnabled()) {
				logger.info("任务[{}]开始运行时间[{}]", task.getName(), new Date());
			}
			try {
				task.run();
			...
			if (logger.isInfoEnabled()) {
				logger.info("任务[{}]结束运行时间[{}]", task.getName(), new Date());
			}
		}
